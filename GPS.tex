...
\begin{frame}{POWERPLAY}
  ...
  (ii) BUT, problem-solver' \emph{can} solve the problem
  ...
  \messageOverlay{Important point:\\
  a ``new problem'' can also be just solving old problems more efficiently}
\end{frame}

\begin{frame}{POWERPLAY on a recurrent network}

  \includegraphics[width = 0.9\textwidth]{figures/powerplay}

  \pause

  \messageOverlay{finding a new problem-solver (network):\\
...
\begin{frame}{POWERPLAY demo}

  Learn to move around a ``fovea.'' 24-dimensional input, 8 outputs (cardinal direction+step size)

  \includegraphics[width = 0.8\textwidth]{figures/powerPlayFovea.png}

\end{frame}

\begin{frame}{Sparsification (+ spontaneous modularization)}
  Typically $\approx 50\%$ of network weights used for each task
  
\centering  \includegraphics[width = 0.9\textwidth]{figures/ppcompression}
  
\end{frame}

\begin{frame}{Words of caution}
  ...
  \emph{(from the conclusion of the POWERPLAY paper)}
\end{frame}
