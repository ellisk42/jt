...
%    \node[ultra thick,anchor = west,inner sep=0pt,label=below:{\small Similarity}](similarity) at (11.3,0) {\includegraphics[width = 1cm]{TikZfigures/expert-38-trim.png}};
    ...
    \pause
    \draw[->, very thick, red] ([yshift = -15,xshift=3]observation.east) to[out = -30,in = -150] node[midway,yshift = -25,xshift=0]{{\small \begin{tabular}{l}
...
\begin{frame}{Top-down influences on perception}
  \Wider[2em]{
    ...
    \multicolumn{3}{c}{\only<7>{\includegraphics[width = 0.9\textwidth]{TikZfigures/programSuccess71_statement.png}}}
...
\begin{frame}[fragile]{Abstraction Sleep: Growing the library via refactoring}
  ...
  \begin{tikzpicture}[scale=1]
    ...
    \node[anchor=west] at (-5.5,-3.7) {\small cf. %%[Tate,  et al. %% 2009
      [Downey et al. 1980]
      };
...
\Wider[4em]{  \begin{tikzpicture}
    ...
    \node[align=left,anchor=west] at (-5.75,-5) {\small cf. Downey et al. 1980\\\small\phantom{cf. }Gulwani 2012\\\small\phantom{cf. }Liang et al. 2010};
...
\end{tikzpicture}}
  \myPaperRight{\small Ellis, Wong, Nye, ..., Solar-Lezama, Tenenbaum. 2020.}
\end{frame}

\begin{frame}[fragile]{}
  \myPaperRight{\small Ellis, Wong, ..., Solar-Lezama, Tenenbaum. 2020.}%
...
\begin{frame}{Synergy between dreaming and library learning}
  \begin{tikzpicture}[scale=1.5]
    ...
    \visible<2->{
      ...
      \draw [->,thick,dashed] (Library.north) to[out = 90,in = 180] node[fill=white,inner sep=0pt,align = center]{  \footnotesize{Inductive bias}\\\footnotesize{(Wake)}} (synthesis.west);
    }
    \visible<3->{
...
\multicolumn{2}{c}{    \visible<2->{\includegraphics[height = 1cm]{../dreamcoder/figures/learningCurves/revision/scatterLegend.png}}}
    \end{tabular}
...
\begin{frame}{Growing a language for recursive programming}
  \myPaper{\small Ellis, Wong, Nye, ..., Solar-Lezama, Tenenbaum. 2020.}\Wider[5em]{\only<1>{\includegraphics[width = \textwidth]{figures/mcCarthyAnimation5}}
...
\begin{frame}{}
  What we've got now:\\ a toolkit for program induction,\\addressing combinatorial program search via learning, integrating symbolic, probabilistic, and neural techniques
