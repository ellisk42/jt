...
\institute{Synthesis of Models and Systems}
\date{2021}
...
%      \node(l)[draw] at (2,-1) {\texttt{$\lambda$ (a)}};
      ...
      \only<5->{
        ...
        \draw[->] (m.south)--node[sloped, above]{\tiny P($\cdot \vert$ \texttt{*},\text{arg}=\text{right})}(x1.north);
        \draw[->] (m.south)--node[sloped, above]{\tiny P($\cdot \vert$ \texttt{*},\text{arg}=\text{left})}(x2.north);
      }
