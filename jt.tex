...
\begin{frame}{}
  ...
  &\visible<3->{= \text{P}(\text{rollout satisfies $goal$}~|~\eval{pp}, goal)}\\
...
&  \mathcal{L}^{\text{RL}} = ~ R  \sum_{t\leq T}\log V(\eval{pp_t},goal) + (1 - R) \sum_{t\leq T}\log (1 - V(\eval{pp_t},goal))\\&\phantom{test} + R\sum_{t\leq T}\log \pi(a_t|\eval{pp_t},goal)
  \end{align*}}
  \vspace*{-1.5cm}\Wider[5em]{
    \begin{center}
      \includegraphics[width = 0.9\textwidth]{figures/capitalism.png}
