...
%  \Wider[5em]{
    \textbf{Program induction}: %% roots in the works of Solomonoff, Fodor, McCarthy.
    agents that learn by writing their own code
...
\begin{frame}{Why will it work this time?}
  ...
  better toolkits:
  \begin{itemize}
  \item \textbf{probabilistic} methods for uncertainty and learning-to-learn
  \item \textbf{neural} methods for guiding combinatorial search
  \item \textbf{symbolic} methods, from the \textbf{programming languages} community
    \begin{itemize}
  ...
  better problems:
  \begin{itemize}
  \item inverse perception, analysis-by-synthesis
  \item learning models
  \item learning to learn
  \item natural language$\to $code
  \item programming by examples, computer-aided-programming
  \end{itemize}
...
\begin{frame}[fragile]{Perception, Synthesizing models, Learning-to-Learn}
  ...
  \only<2>{Theme \#2: Synthesizing human-understandable models}
  ...
  \only<3->{Theme \#3: Learning to synthesize programs}
...
%    \visible<4->{\hspace*{-1cm}\includegraphics[width = 0.4\textwidth]{figures/bg.png}}&
 ...
 %   \visible<6->{\includegraphics[width = 0.3\textwidth]{figures/fractal_tree}}&
  ...
  \visible<2->{
    \messageOverlay{why?\\\phantom{te}impute missing objects, extrapolate percepts,\\\phantom{te}learn visual concepts (`arch', `spiral', `Ising model'),\\\phantom{te}assist graphic design, assist 3D modeling\\how?\\\phantom{te}Bayesian inference of graphics program conditioned on image,\\\phantom{te}+program synthesis\\\phantom{te}+learning}
  }
\end{frame}
...
\begin{frame}{Example programs}
 \includegraphics[width = 0.5\textwidth]{exampleTikZ.pdf}
  \pause
...
\begin{frame}{57 languages drawn from 70 datasets}
  \includegraphics[width = \textwidth]{phonology/figures/languageComparison}
\end{frame}
...
\newcommand{\chooseAny}[3]{
  \node[anchor=north](any) at (#1) {\textsc{\small Any}};
  \node(l) at ([xshift=-0.25cm,yshift=-0.5cm]any.south) {#2};
  \node(r) at ([xshift=0.25cm,yshift=-0.5cm]any.south) {#3};
  \draw (any.south)--(l.north);
  \draw (any.south)--(r.north);  
}
\newcommand{\chooseTreeTiny}[3]{
  ...
  \chooseAny{[yshift=-0.2cm]a#1.south}{#3}{$z$}

  \draw (a#1.south) -- (any.north);
  \draw (argument#1.north) -- (ar#1.south);
  \draw (a#1.north) -- (ar#1.south);
}  
...
\newcommand{\chooseTreeSmall}[7]{
  ...
  \node[anchor=north](r#1) at ([xshift=0.5cm,yshift=-0.5cm]root#1) {#5};
  \node(l#1) at ([xshift=-0.5cm,yshift=-0.5cm]root#1) {};
  \chooseAny{[xshift=-0.5cm,yshift=-0.5cm]l#1}{#3}{#4};
  \draw (l#1.north) -- (any.north);
...
\newcommand{\chooseTree}[6]{
  \node(root#1) at (#2) {};
  \chooseAny{[xshift=0.75cm,yshift=-0.75cm]root#1}{#4}{#5};
  \draw (root#1.south) -- (any.north);  
  \node(l#1) at ([xshift=-0.75cm,yshift=-0.75cm]root#1) {};
  \node(ll#1) at ([xshift=-0.75cm,yshift=-0.75cm]l#1) {#3};
  \chooseAny{[xshift=0.5cm,yshift=-0.5cm]l#1}{#4}{#5};
  \draw (l#1.north) -- (any.north);
...
\begin{frame}[fragile]{version space}
...
\begin{frame}{Programmatic models of the physical world}
  \begin{tabular}{rcc}
    \begin{tabular}{c}
      hinge
    \end{tabular}&
    ...
    \begin{tabular}{c}
      ...
      \end{tabular}&
    \begin{tabular}{c}
      \includegraphics[width = 3cm]{figures/gear2}
      \end{tabular}\\
    \begin{tabular}{c}
      doorknob
    \end{tabular}&
    ...
    \end{tabular}&
    \begin{tabular}{c}
      \includegraphics[width = 3cm]{figures/oldTheDoorknobs.jpg}
    \end{tabular}
  \end{tabular}
\end{frame}
