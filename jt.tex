...
\begin{frame}[t]{}
  \Wider[3em]{
    \only<1>{OBJECTIVE: $\probability\left[\text{library} \right]\prod\limits_{\text{task}}\probability\left[\text{task}|\text{library} \right]$}
    \only<2->{OBJECTIVE: $\probability\left[\text{library} \right]\prod\limits_{\text{task}}\sum\limits_{\text{prog}}\probability\left[\text{task}|\text{prog} \right]\probability\left[\text{prog}|\text{library} \right]$}\\  
    \visible<3->{\phantom{OBJECTIVE: }$\qquad\geq \probability\left[\text{library} \right]\prod\limits_{\text{task}}\sum\limits_{\substack{\text{prog}\in \\\text{beam}(\text{task})}}\probability\left[\text{task}|\text{prog} \right]\probability\left[\text{prog}|\text{library} \right]$}\\
    \visible<4->{lower bound holds $\forall \text{beam}(\cdot )$.}
    \visible<5->{ finite beam: $|\text{beam}(\text{task})|\leq 5$}\\\\

    \visible<6->{\underline{ABSTRACTION}: max wrt library:}\\
    \visible<7->{$\argmax\limits_{\text{library}}\underbrace{\probability\left[\text{library} \right]}_{\text{short lib}}\prod\limits_{\text{task}}\underbrace{\sum\limits_{\substack{\text{prog}\in \\\text{beam}(\text{task})}}\probability\left[\text{task}|\text{prog} \right]\probability\left[\text{prog}|\text{library} \right]}_{\text{compress programs}}$}\\
    \visible<8->{\underline{WAKE}: max wrt beams:}\\
    \only<9>{$\text{beam}(\text{task}) = \argmaxk\limits_{\text{prog}}\underbrace{\probability\left[\text{task}|\text{prog} \right]\probability\left[\text{prog}|\text{library} \right]}_{\propto\text{posterior prob.}}$}
    \visible<10->{$\text{beam}(\text{task}) = \argmaxk\limits_{\text{prog}\sim\text{NN}(\cdot |\text{task})}\underbrace{\probability\left[\text{task}|\text{prog} \right]\probability\left[\text{prog}|\text{library} \right]}_{\propto\text{posterior prob.}}$}\\
    \visible<11->{\underline{DREAMING}: train neural net (NN):}\\
    \visible<11->{train $\text{NN}(\text{prog}|\text{task})\approx\probability\left[\text{prog}|\text{task},\text{library} \right]\propto\probability\left[\text{task}|\text{prog} \right]\probability\left[\text{prog}|\text{library} \right]$}

    \only<1-5>{
     % \vfill
      \vspace*{-4cm}\hspace*{4cm}\includegraphics[width = 0.3\textwidth]{cycleGraphicalModel1}
    }
  }
\end{frame}
...
%  \myPaper{\footnotesize Ellis, Wong, Nye, ..., Solar-Lezama, Tenenbaum. 2020.}
  ...
  \only<2>{\includegraphics[width = \textwidth]{cycleGraphicalModelAbstraction}}
\end{frame}
