...
\begin{frame}{The premise of program induction}
  ...
  2. Learning$ = $adding to that body of knowledge$ = $\\\phantom{2. }making new programs$ = $program synthesis
...
\begin{frame}{}
\Wider[4em]{  \vspace*{-0.9cm}\begin{align*}
    &pp = \text{partial program (state)}\phantom{test}\sem{pp} = \text{render of }pp\nonumber\\
    &\visible<2->{a = \text{action (line of code). sample }\left\{a_t \right\}_{t = 1}^T\text{, define }pp_t = a_{t - 1}(pp_{t - 1})}\nonumber\\
    \visible<3->{&\mathcal{L}^{\text{imitation}} = \sum_{t = 1}^{T}\log\pi(a_t|pp_{t - 1},\sem{pp_T})}\nonumber
          %% v\left( \eval{pp}, spec\right) &= \E_\pi\left[R~|~\eval{pp}, spec \right] = \E_\pi\left[\1{spec\text{ is satisfied}}~|~\eval{pp}, spec \right]\nonumber\\ &= \text{P}(\text{rollout w/ $\pi$ satisfies $spec$}~|~\eval{pp}, spec)\\
          %% \mathcal{L}^{\text{RL}}(v,\pi) = ~ &R  \sum_{t\leq T}\log v(\eval{pp_t},spec) + (1 - R) \sum_{t\leq T}\log (1 - v(\eval{pp_t},spec)) + R\sum_{t\leq T}\log \pi(a_t|\eval{pp_t},spec)
  \end{align*}}
  \vspace*{-1.5cm}\Wider[5em]{
    \begin{center}
      \includegraphics[width = 0.9\textwidth]{figures/capitalism.png}
    \end{center}
  }
%  \myPaper{\tiny Ellis$^*$, Nye$^*$, Pu$^*$, Sosa$^*$, Tenenbaum, Solar-Lezama. NeurIPS 2019.\\$^*$equal contribution}
\end{frame}
\begin{frame}{}
  \Wider[4em]{
    \vspace*{-0.9cm}\begin{align*}
      &pp = \text{partial program (state)}\phantom{tes}\sem{pp} = \text{render of }pp\phantom{tes}a = \text{line of code (action)}\nonumber\\
      &V\left( \eval{pp}, goal\right) = \E_\pi\left[R~|~\eval{pp}, goal \right]\visible<2->{ = \E_\pi\left[\1{goal\text{ is satisfied}}~|~\eval{pp}, goal \right]}\nonumber\\
  &\visible<3->{\phantom{V\left( \eval{pp}, goal\right)}= \text{P}(\text{rollout satisfies $goal$}~|~\eval{pp}, goal)}\\
  %% \mathcal{L}^{\text{RL}}(V,\pi) = ~ &R  \sum_{t\leq T}\log V(\eval{pp_t},goal) + (1 - R) \sum_{t\leq T}\log (1 - V(\eval{pp_t},goal)) + R\sum_{t\leq T}\log \pi(a_t|\eval{pp_t},goal)
  \end{align*}}
  \vspace*{-1.5cm}\Wider[5em]{
    \begin{center}
      \includegraphics[width = 0.9\textwidth]{figures/capitalism.png}
...
\begin{frame}{}
  \Wider[4em]{
    \vspace*{-0.9cm}\begin{align*}
      &pp = \text{partial program (state)}\phantom{tes}\sem{pp} = \text{render of }pp\phantom{tes}a = \text{line of code (action)}\nonumber\\
      &V\left( \eval{pp}, goal\right) = \text{P}(\text{rollout satisfies $goal$}~|~\eval{pp}, goal)\nonumber\\
&  \visible<2->{\mathcal{L}^{\text{RL}} = ~ R  \sum_{t\leq T}\log V(\eval{pp_t},goal) + (1 - R) \sum_{t\leq T}\log (1 - V(\eval{pp_t},goal))}\\&\visible<3->{\phantom{test} + R\sum_{t\leq T}\log \pi(a_t|\eval{pp_t},goal)}
  \end{align*}}
  \vspace*{-1.5cm}\Wider[5em]{
    \begin{center}
      \includegraphics[width = 0.9\textwidth]{figures/capitalism.png}
...
\begin{frame}{}
  \Wider[4em]{
    \vspace*{-0.9cm}\begin{align*}
      &pp = \text{partial program (state)}\phantom{tes}\sem{pp} = \text{render of }pp\phantom{tes}a = \text{line of code (action)}\nonumber\\
      &\text{Sequential Monte Carlo for search:}\\
      &\text{maintain population of particles (states)}\\
      &\visible<2->{\text{1. sample from policy $\pi$ to get new population}}\\
      &\visible<3->{\text{2. reweigh by value $V$, resample, goto 1}}
  \end{align*}}
  \vspace*{-1.5cm}\Wider[5em]{
    \begin{center}
      \includegraphics[width = 0.9\textwidth]{figures/capitalism.png}
...
\begin{comment}
\begin{frame}{}
  What we've got now:\\ a toolkit for program induction,\\addressing combinatorial program search via learning, integrating techniques for machine learning, AI, and programming languages %symbolic, probabilistic, and neural techniques
...
\begin{frame}{}
  ...
  These applications are in reach.
