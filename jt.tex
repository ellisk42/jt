...
\begin{frame}{Why didn't this old idea work?}
  ...
  \textbf{main obstacle: combinatorial search is hard}
\end{frame}
...
%  Program induction is already working: a roadmap of problems...
...
\begin{frame}[fragile]{Perception, Synthesizing models, Learning-to-Learn}
  ...
  \Wider[5em]{
    \includegraphics[width = \textwidth]{statement/taskbar2.png}
    }
\end{frame}
...
\hspace*{-1cm}\begin{tikzpicture}
    \node[draw,label={hand drawing}](picture1) at (0,-1) {\includegraphics[height = 2.2cm]{TikZfigures/expert-31.png}};
...
\begin{frame}[fragile]{Learning to infer graphics programs from hand-drawn images}
  ...
  \hspace*{-1cm}\begin{tikzpicture}
    \node[draw,label={[align=center]hand drawing}](picture1) at (0,-1) {\includegraphics[height = 2.2cm]{TikZfigures/expert-31.png}};
    \node[label={machine-generated program},draw,right=1cm of picture1] (c1) {
      \begin{lstlisting}[basicstyle = \small\ttfamily]
for (0 <= i < 3 <@\textcolor{red}{\textbf{+ 1}}@>)
 ...
 for (0 <= j < i + 1)
  circle(3*i+1,-2*j+5)
    ...
    \node(e)[anchor=west,label={[align=center]autogenerated\\extrapolation}] at ([xshift=1cm]c1.east) {\includegraphics[width = 2.2cm]{TikZfigures/31-extrapolated.png}};
    
    \draw[very thick,->] (c1.east)  -- (e.west);
    %    \node(l1)[anchor=south] at ([yshift=0.75cm,xshift=2.5cm]picture1.north) {model infers program from drawing};
    \node[anchor=south west] at ([yshift=-1cm]picture1.south west) {\textbf{zero-shot generalization / extrapolation}};
  \end{tikzpicture}
\end{frame}
...
%    \node[ultra thick,anchor = west,inner sep=0pt,label=below:{\small Similarity}](similarity) at (11.3,0) {\includegraphics[width = 1cm]{TikZfigures/expert-38-trim.png}};
    ...
    \draw[->, very thick, red] ([yshift = -15,xshift=0]trace.east) to[out = -30,in = -150] node[midway,yshift = -25,xshift=53]{{\small \begin{tabular}{l}
...
\begin{frame}{}
  ...
  \Wider[5em]{
    \only<1>{\includegraphics[width = \textwidth]{figures/capitalism3}}
    \only<2>{\includegraphics[width = \textwidth]{figures/capitalism2}}
    \only<3>{\includegraphics[width = \textwidth]{figures/capitalism1}}
    \only<4>{\includegraphics[width = \textwidth]{figures/capitalism}}
    \vfill
    \small  Ellis$^*$, Nye$^*$, Pu$^*$, Sosa$^*$, Tenenbaum, Solar-Lezama. NeurIPS 2019. $^*$equal contribution
...
\begin{frame}{}
  \Wider[5em]{
    ...
    \only<2>{\includegraphics[width = \textwidth]{figures/betterMontage}}
  }
\end{frame}
...
%  \centering
  ...
  \only<11>{  \Wider[5em]{\includegraphics[width = \textwidth]{figures/figureOneFinal3_exploded}}}
  \only<12->{  \Wider[5em]{\includegraphics[width = \textwidth]{figures/figureOneFinal3}}}
%  \only<12>{  \Wider[5em]{\includegraphics[width = \textwidth]{figures/figureOneFinal4}}}
  ...
  \visible<13>{\normalsize induced sort program found in $\leq 10$min. Brute-force search without learned library would take $\approx 10^{78}$ years }
...
\begin{frame}{DreamCoder Domains}
  \Wider[5em]{
    \only<1>{\includegraphics[width = \textwidth]{statement/taskbar2_expended.png}}
    \only<2>{\includegraphics[width = \textwidth]{statement/taskbar2_selected.png}}
    }
\end{frame}
...
\begin{frame}{LOGO Graphics -- learning an interpretable library of concepts}
  \Wider[5em]{
    \only<1>{\includegraphics[width = \textwidth]{dc/logo_kathy.png}}
    \only<2>{\includegraphics[width = \textwidth]{dc/logo_kathy_exploded.png}}
    \only<3->{\includegraphics[width = \textwidth]{dc/logo_kathy.png}}
...
%  \emph{before} learning:
  \Wider[5em]{
    \begin{center}
      \includegraphics[height=0.9\textheight]{dc/dreams/beforeLearning25September11}
    \end{center}
  }
...
%        \multicolumn{1}{l}{\textbf{B}}\\
    \includegraphics[height = 0.3\textwidth]{figures/depthVersusAccuracy_revision_MEAN.png}&
    \visible<2>{\includegraphics[height = 0.3\textwidth]{figures/depthVersusAccuracy_revision_SIZE.png}} \\\\
...
%    \only<5>{\includegraphics[width = \textwidth]{figures/mcCarthyAnimation1}}
  ...
  \visible<5>{\textbf{\Large 1 year of compute. 5 days on 64 CPUs.}}
...
\begin{frame}{What's in reach}
  What we've got now:\\ a neuro-symbolic toolkit for Bayesian program induction, combining scalable parallelism with insights from the programming languages community
  ...
  \Wider[5em]{
    \begin{center}
      \begin{tabular}[t]{ccc}
        ...
        \closex{0.3}{
          \begin{tabular}{c}
            \only<1>{Modeling the}\only<2>{\textbf{Modeling the}}\\
            \only<1>{physical world}\only<2>{\textbf{physical world}}\\
...
\begin{frame}{Modeling the physical world: Within reach} %$\sim$vaporware$\sim$}
  \Wider[5em]{
    \begin{center}
      \input{vapor.tex}
    \end{center}
...
\begin{frame}{What's far off, but worthwhile?}
  ...
  \Wider[5em]{
    ...
    \end{tabular}}
    \only<2>{
      \includegraphics[width = 0.9\textwidth]{statement/taskbar2.png}
    }
    \end{center}
  }
...
\Wider[6em]{  \begin{tabular}{cccc}
    ...
    \person{Adam Albright}{collaborators/Adam}\\    \phantom{test}\\
    \person{Max Nye}{collaborators/MaxNye}&
    \person{Cathy Wong}{collaborators/cw}&
    \person{Yewen Pu}{collaborators/Evan}&
    \person{Dan Ritchie}{collaborators/Daniel}\\\phantom{test}\\
